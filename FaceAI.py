...
def face_detect():
    ...
    print("Название файла - ", myList)
    classNames.clear()

    for cls in myList:
        stream = open(f"{path}/{cls}", 'rb')
        bytes = bytearray(stream.read())
        array = np.asarray(bytes, dtype=np.uint8)
        curImg = cv2.imdecode(array, cv2.IMREAD_UNCHANGED)
        images.append(curImg)
        classNames.append(os.path.splitext(cls)[0])

    print("Имя человека - ", classNames)
    ...
    while True:
        ...
        for encodeFace, faceLoc in zip(encodeCurFrame, facesCurFrame):
            ...
            if matches[matchIndex]:
                ...
                filename = 'KnownFaces/Unknown.jpg'
                cv2.imwrite(filename, img)
                print("Лицо сохранено")
                find_clone(filename)

                
        cv2.imshow("FaceAI", img)
